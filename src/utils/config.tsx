// HCG Supply Chain Analytics backend (FastAPI, parquet-backed).
// Override per-environment via NEXT_PUBLIC_DASHBOARD_API; defaults to local backend.
const DASHBOARD_API_BASE_URL =
  process.env.NEXT_PUBLIC_DASHBOARD_API?.trim() || "http://localhost:8000";

// Chat-AI backend removed from the POC. Kept as an empty export for any legacy
// import sites until the chat module is fully deleted in the frontend rebase.
const ANALYTICS_AI_API_BASE_URL = "";

export { ANALYTICS_AI_API_BASE_URL, DASHBOARD_API_BASE_URL };

import styled from 'styled-components';
export const StyledWrapper = styled.div`
  .wrapper {
  transform: scale(0.3); /* Adjust scale to make smaller */
  transform-origin: top left; /* Optional: sets scaling point */
  width: fit-content;
  height: fit-content;
  margin-left:30px;
  margin-top:-30px;
...
.pill {
  background: transparent;
  width: 8vmin;       /* Smaller width */
  height: 25vmin;     /* Smaller height */
  display: flex;
  align-items: center;
  ...
  .pill .side:after {
    ...
    left: 0;
    border-radius: 6vmin 6vmin 0 0;
    border: 1.75vmin solid #244a6c33;
