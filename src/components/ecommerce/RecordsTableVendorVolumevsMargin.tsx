...
interface StockChangeResponse {
  ...
  "Unit Purchase Price":string,
  "Unit MRP Price":string,
  "Unit Margin": string;
}
...
export default function VendorVolumevsMarginTable() {
  ...
  const transformApiData = (data: StockChangeResponse[], startIndex = 0): StockChangeRow[] => {
    return data.map((entry, index) => ({
      ...
      grnVolume: parseFloat(entry["GRN Volume"]) || 0,
      averageUnitCost: parseFloat(entry["Unit Purchase Price"]) || 0,
      sellingPrice: parseFloat(entry["Unit MRP Price"]) || 0,
      margin: parseFloat(entry["Unit Margin"]) || 0,
    }));
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `${DASHBOARD_API_BASE_URL}/kpi/vendor-volume-vs-margin-table?Plant=${regionName}&page=${pagination.pageIndex}&page_size=${pagination.pageSize}&${query}`;
      ...
      console.log("Recieved Data Table : ", responseData)
  ...
  }, [pagination.pageIndex, pagination.pageSize, columnFilters, sorting, selectedRegion]);
  ...
  const exportToCSV = async () => {
    ...
    const url = `${DASHBOARD_API_BASE_URL}/kpi/vendor-volume-vs-margin-table?Plant=${regionName}&page=0&page_size=100000&${queryParams}`;
    ...
    try {
      ...
      link.href = blobUrl;
      link.setAttribute("download", "vendor-volume-vs-margin-table.csv");
  ...
  // },
   ...
   muiTableProps: {
  sx: {
    tableLayout: "auto", // or remove this line completely
  },
},
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white px-5 pb-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
      <div className="flex flex-col gap-5 mb-6 sm:flex-row sm:items-center sm:justify-between flex-wrap">
        <h2 className="text-xl text-gray-500 dark:text-white sm:flex-1">
          Vendor Performance Volume vs Margin Table
        </h2>
      </div>
