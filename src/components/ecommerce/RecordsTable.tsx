...
export default function StockChangeTable() {
  ...
  const transformApiData = (data: StockChangeResponse[], startIndex = 0): StockChangeRow[] => {
    return data.map((entry, index) => ({
      ...
      stockChange: parseFloat(entry["Stock Change"]) || 0,
      amountChange: parseFloat(entry["Amount in LC Change"]) || 0,
    }));
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `${DASHBOARD_API_BASE_URL}/kpi/stock-change-table?Plant=${regionName}&page=${pagination.pageIndex}&page_size=${pagination.pageSize}&${query}`;
  ...
  const exportToCSV = async () => {
    ...
    const url = `${DASHBOARD_API_BASE_URL}/kpi/stock-change-table?Plant=${regionName}&page=0&page_size=100000&${queryParams}`;
