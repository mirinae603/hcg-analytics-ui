...
export default function StockChangeTable() {
  ...
  const transformApiData = (data: StockChangeResponse[], startIndex = 0): StockChangeRow[] => {
    return data.map((entry, index) => ({
      ...
      stockChange: parseFloat(entry["Closing Quantity"]) || 0,
      amountChange: parseFloat(entry["Inventory Valuation"]) || 0,
    }));
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `http://localhost:8000/kpi_2_table?Plant=CHENNAI&page=${pagination.pageIndex}&page_size=${pagination.pageSize}&${query}`;
  ...
  const exportToCSV = async () => {
    ...
    const url = `http://localhost:8000/kpi_2_table?Plant=CHENNAI&page=0&page_size=100000&${queryParams}`;
