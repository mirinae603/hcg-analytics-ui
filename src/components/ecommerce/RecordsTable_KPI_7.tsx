...
export default function RecordsTable_KPI_7() {
  ...
  const transformApiData = (data: StockChangeResponse[], startIndex = 0): StockChangeRow[] => {
    return data.map((entry, index) => ({
      ...
      totalVolume: entry["Total Volume"],
      averageUnitCost: entry["Average Unit Cost"],
      sellingPrice: entry["Selling Price"] || 0,
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `http://localhost:8000/kpi_7_table?Plant=CHENNAI&page=${pagination.pageIndex}&page_size=${pagination.pageSize}&${query}`;
  ...
  const exportToCSV = async () => {
    ...
    const url = `http://localhost:8000/kpi_7_table?Plant=CHENNAI&page=0&page_size=100000&${queryParams}`;
    ...
    <div className="rounded-2xl border border-gray-200 bg-white px-5 pb-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
      <div className="flex flex-col gap-5 mb-6 sm:flex-row sm:items-center sm:justify-between flex-wrap">
        <h2 className="text-xl text-gray-800 dark:text-white sm:flex-1">
          Vendor Performance -  Volume vs Unit Margin
        </h2>
      </div>
