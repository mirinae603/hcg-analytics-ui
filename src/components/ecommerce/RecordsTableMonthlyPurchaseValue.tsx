...
export default function MonthlyPurchaseValueTable() {
  ...
  const transformApiData = (data: StockChangeResponse[], startIndex = 0): StockChangeRow[] => {
    return data.map((entry, index) => ({
      ...
      monthlyPurchaseValue: parseFloat(entry["Monthly Purchase Value"]) || 0,
    }));
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.log("Sending filter model:", { columnFilters, sorting, globalFilter });

      const url = `${DASHBOARD_API_BASE_URL}/kpi/monthly-purchase-value-table?Plant=${regionName}&page=${pagination.pageIndex}&page_size=${pagination.pageSize}&${query}`;
  ...
  const exportToCSV = async () => {
    ...
    const url = `${DASHBOARD_API_BASE_URL}/kpi/monthly-purchase-value-table?Plant=${regionName}&page=0&page_size=100000&${queryParams}`;
    ...
    try {
      ...
      link.href = blobUrl;
      link.setAttribute("download", "inventory-turnover-ratio-table.csv");
  ...
  () => [
    ...
    {
      accessorKey: "monthlyPurchaseValue",
      header: "Monthly Purchase Value",
      minSize: 150,
      grow: 2,
      filterVariant: "range",
