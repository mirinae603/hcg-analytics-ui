...
export default function RecordsTable_KPI_3() {
  ...
  const transformApiData = (data: StockChangeResponse[], startIndex = 0): StockChangeRow[] => {
    return data.map((entry, index) => ({
      ...
      unitsSold: parseFloat(entry["Total Units Sold"]) || 0,
    }));
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `http://localhost:8000/kpi_8_table?Plant=CHENNAI&page=${pagination.pageIndex}&page_size=${pagination.pageSize}&${query}`;
  ...
  const exportToCSV = async () => {
    ...
    const url = `http://localhost:8000/kpi_8_table?Plant=CHENNAI&page=0&page_size=100000&${queryParams}`;
