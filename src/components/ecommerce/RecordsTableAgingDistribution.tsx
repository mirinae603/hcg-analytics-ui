...
interface StockChangeResponse {
  ...
  "Age Since Last Sale": string;
  "Age Since Last Purchase":string,
  "Aging":string,
}
...
interface StockChangeRow {
  ...
  materialId: string;
  materialName: string;
  ageSinceLastSale: number;
  ageSinceLastPurchase: number;
  aging: string;
}
...
export default function AgingDistributionTable() {
  ...
  const transformApiData = (data: StockChangeResponse[], startIndex = 0): StockChangeRow[] => {
    return data.map((entry, index) => ({
      ...
      ageSinceLastSale: parseFloat(entry["Age Since Last Sale"]) || 0,
      ageSinceLastPurchase: parseFloat(entry["Age Since Last Purchase"]) || 0,
      aging: entry["Aging"],
    }));
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `${DASHBOARD_API_BASE_URL}/kpi/inventory-aging-table?Plant=${regionName}&page=${pagination.pageIndex}&page_size=${pagination.pageSize}&${query}`;
  ...
  const exportToCSV = async () => {
    ...
    const url = `${DASHBOARD_API_BASE_URL}/kpi/return-rate-table?Plant=${regionName}&page=0&page_size=100000&${queryParams}`;
    ...
    try {
      ...
      link.href = blobUrl;
      link.setAttribute("download", "return-rate-table.csv");
  ...
  () => [
    ...
    {
      accessorKey: "ageSinceLastPurchase",
      header: "LastPurchase Age",
      minSize: 150,
      grow: 2,
      filterVariant: "range",
      muiTableBodyCellProps: { align: "right" },
    ...
    },
    {
      ...
      filterVariant: "text",
      muiFilterTextFieldProps: {
        placeholder: "Filter by Aging Category",
        fullWidth: true,
      },
    },
