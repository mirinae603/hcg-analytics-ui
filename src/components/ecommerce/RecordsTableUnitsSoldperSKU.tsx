...
export default function UnitSoldperSKUTable() {
  ...
  const transformApiData = (data: StockChangeResponse[], startIndex = 0): StockChangeRow[] => {
    return data.map((entry, index) => ({
      ...
      totalUnitsSold: parseFloat(entry["Total Units Sold"]) || 0,
    }));
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `${DASHBOARD_API_BASE_URL}/kpi/unit-sold-per-sku-table?Plant=${regionName}&page=${pagination.pageIndex}&page_size=${pagination.pageSize}&${query}`;
  ...
  const exportToCSV = async () => {
    ...
    const url = `${DASHBOARD_API_BASE_URL}/kpi/unit-sold-per-sku-table?Plant=${regionName}&page=0&page_size=100000&${queryParams}`;
    ...
    try {
      ...
      link.href = blobUrl;
      link.setAttribute("download", "unit-sold-per-sku-table.csv");
