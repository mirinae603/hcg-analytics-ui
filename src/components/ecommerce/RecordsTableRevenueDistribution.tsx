...
export default function RevenueDistributionTable() {
  ...
  const transformApiData = (data: StockChangeResponse[], startIndex = 0): StockChangeRow[] => {
    return data.map((entry, index) => ({
      ...
      year: entry["Year"],
      period: entry["Month"],
      opSales: parseFloat(entry["OP Sales"]) || 0,
      ipIssue: parseFloat(entry["IP Issue"]) || 0,
      internalConsumption: parseFloat(entry["Internal Consumption"]) || 0,
    }));
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `${DASHBOARD_API_BASE_URL}/kpi/revenue-distribution-table?Plant=${regionName}&page=${pagination.pageIndex}&page_size=${pagination.pageSize}&${query}`;
  ...
  const exportToCSV = async () => {
    ...
    const url = `${DASHBOARD_API_BASE_URL}/kpi/revenue-distribution-table?Plant=${regionName}&page=0&page_size=100000&${queryParams}`;
    ...
    try {
      ...
      link.href = blobUrl;
      link.setAttribute("download", "revenue-distribution-table.csv");
  ...
  () => [
    ...
    },
   {
    ...
    {
      accessorKey: "internalConsumption",
      header: "Internal Consumption",
      minSize: 150,
      grow: 2,
      filterVariant: "range",
      muiTableBodyCellProps: { align: "right" },
