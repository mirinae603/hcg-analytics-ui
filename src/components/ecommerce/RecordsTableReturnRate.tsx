...
export default function ReturnRateTable() {
  ...
  const transformApiData = (data: StockChangeResponse[], startIndex = 0): StockChangeRow[] => {
    return data.map((entry, index) => ({
      ...
      soldUnits: parseFloat(entry["Sold Units"]) || 0,
      returnedUnits: parseFloat(entry["Returned Units"]) || 0,
      returnRate: parseFloat(entry["Return Rate (%)"]) || 0,
    }));
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `${DASHBOARD_API_BASE_URL}/kpi/return-rate-table?Plant=${regionName}&page=${pagination.pageIndex}&page_size=${pagination.pageSize}&${query}`;
