...
interface StockChangeResponse {
  ...
  "Cumulative Purchase Value": string;
}
...
export default function RecordsTable_KPI_6() {
  ...
  const transformApiData = (data: StockChangeResponse[], startIndex = 0): StockChangeRow[] => {
    return data.map((entry, index) => ({
      ...
      cumulativePurchaseValue: parseFloat(entry["Cumulative Purchase Value"]) || 0,
    }));
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `http://localhost:8000/kpi_6_table?Plant=CHENNAI&page=${pagination.pageIndex}&page_size=${pagination.pageSize}&${query}`;
      ...
      console.log("KPI 6 Recieved Table Data ", responseData);
  ...
  const exportToCSV = async () => {
    ...
    const url = `http://localhost:8000/kpi_6_table?Plant=CHENNAI&page=0&page_size=100000&${queryParams}`;
  ...
  () => [
    ...
    {
      accessorKey: "cumulativePurchaseValue",
      header: "Cumulative Purchase Value",
      minSize: 170,
      grow: 2,
      filterVariant: "range",
      muiTableBodyCellProps: { align: "right" },
