...
export default function StockChangeChart() {
  ...
  const fetchData = async () => {
    try {
      ...
      const baseUrl = `${DASHBOARD_API_BASE_URL}/kpi/inventory-valuation`;
...
if (selectedTab === "optionOne") {
  ...
  const month = monthRaw ? toShortMonth(monthRaw) : undefined; // <-- fix here
  const rawValue = entry["Inventory Valuation"];
