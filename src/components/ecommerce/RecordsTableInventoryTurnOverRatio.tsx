...
export default function InventoryTurnOverRatioTable() {
  ...
  const transformApiData = (data: StockChangeResponse[], startIndex = 0): StockChangeRow[] => {
    return data.map((entry, index) => ({
      ...
      averageInventoryValue: parseFloat(entry["Avg Inventory Value"]) || 0,
      inventoryTurnOverRatio: parseFloat(entry["ITR"]) || 0,
    }));
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `${DASHBOARD_API_BASE_URL}/kpi/inventory-turnover-ratio-table?Plant=${regionName}&page=${pagination.pageIndex}&page_size=${pagination.pageSize}&${query}`;
  ...
  const exportToCSV = async () => {
    ...
    const url = `${DASHBOARD_API_BASE_URL}/kpi/inventory-turnover-ratio-table?Plant=${regionName}&page=0&page_size=100000&${queryParams}`;
  ...
  () => [
    ...
    {
      accessorKey: "averageInventoryValue",
    ...
    {
      accessorKey: "inventoryTurnOverRatio",
      header: "ITR",
      minSize: 150,
      grow: 2,
      filterVariant: "range",
