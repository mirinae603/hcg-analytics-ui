...
export default function VendorLeadTimeDetail() {
  ...
  useEffect(() => { setData(null); fetch(`${DASHBOARD_API_BASE_URL}/kpi/vendor-lead-time/insights?Plant=${encodeURIComponent(region)}`).then((r) => r.json()).then(setData).catch(() => setData(null)); }, [region]);
